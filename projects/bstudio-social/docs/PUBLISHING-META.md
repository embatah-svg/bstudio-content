...
```
https://raw.githubusercontent.com/<utente>/<repo>/<branch>/projects/bstudio-social/instagram/2026-06-08_tag01/_export/post-tag01-carousel-slide01.png
```
Alternative: Cloudinary (free tier), Cloudflare R2, un bucket S3, o qualsiasi hosting statico. `ASSET_BASE_URL` punta alla RADICE del repo: gli script aggiungono da soli il prefisso `projects/bstudio-social/`.
...
IG_USER_ID=178414...
ACCESS_TOKEN=EAAG...
ASSET_BASE_URL=https://raw.githubusercontent.com/<utente>/<repo>/<branch>
```
...
Lo script `scripts/publish-instagram.mjs` legge `scripts/publish-queue.json` (un post per voce) e pubblica.

```bash
# dalla cartella projects/bstudio-social/ del repo:
# carousel/reel del giorno indicato nella coda
node scripts/publish-instagram.mjs --tag tag01
...
node scripts/publish-instagram.mjs --tag tag01 --dry-run
```
...
Ho generato una coda di esempio per i 7 giorni hero in `scripts/publish-queue.example.json`. Copiala in `publish-queue.json`, riempi gli URL (o lascia che `ASSET_BASE_URL` li costruisca) e metti `"status":"ready"`. Struttura di una voce:
```json
{
